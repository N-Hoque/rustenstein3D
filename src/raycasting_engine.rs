...
impl REngine {
    ...
    fn update_events(&mut self, event_handler: &EventHandler) -> () {
        ...
        let r#move: f32 = match event_handler.has_mouse_moved_event() {
        ...
        self.vector_direction.x =
            self.vector_direction.x * (r#move).cos() - self.vector_direction.y * (r#move).sin();
        self.vector_direction.y =
            old_dir_x * (r#move).sin() + self.vector_direction.y * (r#move).cos();
        let old_cam_plane_x = self.cam_plane.x;
        self.cam_plane.x = self.cam_plane.x * (r#move).cos() - self.cam_plane.y * (r#move).sin();
        self.cam_plane.y = old_cam_plane_x * (r#move).sin() + self.cam_plane.y * (r#move).cos();
    }
